...
function Sidebar() {
  ...
  return (
    <>
      ...
      <Box 
        height={'100vh'} borderRight={'1px solid'} borderColor={'whiteAlpha.300'} paddingY={8}
        position={'sticky'} top={0} left={0} paddingX={{base: '2', md: '4'}}
      >
        ...
        <Flex direction={'column'} gap={10} width={'full'}>
          ...
          <Link to={'/'} as={RouterLink} paddingLeft={2} display={{base: 'block', md: 'none'}} cursor={'pointer'}
            borderRadius={6} _hover={{bg: 'whiteAlpha.200'}} width={10}
          >
            <InstagramMobileLogo />
          </Link>
          ...
          <Tooltip hasArrow label={'Cerrar Sesion'} placement='right' marginLeft={1} 
            openDelay={500} display={{base: 'block', md: 'none'}}
          >
            <Flex onClick={handleLogOut} alignItems={'center'} gap={4} _hover={{backgroundColor: 'whiteAlpha.400'}} 
              borderRadius={6} padding={2} width={{base: '10', md: 'full'}} marginTop={'auto'} justifyContent={{ base: 'center', md: 'flex-start' }}
            >
              <BiLogOut size={25} />
